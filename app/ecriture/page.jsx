...
const STORAGE_KEY = "lecture6e_writing_practice_v1";

const writingSituations = [
  {
    id: "opinion",
    title: "Texte d’opinion",
    task: "Écris un texte pour donner ton opinion sur le sujet lié au texte lu. Appuie ton opinion avec au moins deux raisons et des exemples.",
  ...
  },
  {
    ...
    task: "Écris un texte pour expliquer un phénomène, une situation ou une idée importante du texte lu. Utilise des informations du texte.",
  ...
  },
  {
    ...
    task: "Écris un texte pour expliquer ta réaction ou ton appréciation du texte. Appuie ton point de vue avec des passages ou des éléments précis.",
...
export default function WritingPage() {
  ...
  const [purpose, setPurpose] = useState("convaincre ou expliquer clairement mon idée");
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setPurpose(saved.purpose || "convaincre ou expliquer clairement mon idée");
      ...
      <div className="grid cols">
        ...
        <section className="card">
          {step === 1 && <div>
            ...
            <div className="card yellow"><b>Consigne :</b><p>{situation.task}</p></div>
            <label>À qui j’écris ?</label>
            <input value={audience} onChange={(e) => setAudience(e.target.value)} />
            <label>Pourquoi j’écris ?</label>
            <input value={purpose} onChange={(e) => setPurpose(e.target.value)} />
            <ul><li>Je repère le sujet.</li><li>Je repère mon destinataire.</li><li>Je repère le but de mon texte.</li><li>Je pense aux informations du texte source que je peux réutiliser.</li></ul>
          </div>}

          {step === 2 && <div>
            <h2>Étape 2 — Je prépare mes idées</h2>
            <p>Écris des idées courtes. Tu peux utiliser le texte source, tes connaissances et tes réactions.</p>
            {ideas.map((row, i) => <div className="card" key={i}>
              <h3>Idée {i + 1}</h3>
              <label>Ce que je sais ou comprends</label><input value={row.know} onChange={(e) => updateIdea(i, "know", e.target.value)} />
              <label>Ce que je veux dire</label><input value={row.say} onChange={(e) => updateIdea(i, "say", e.target.value)} />
              <label>Preuve, exemple ou information du texte</label><input value={row.proof} onChange={(e) => updateIdea(i, "proof", e.target.value)} />
            </div>)}
          </div>}
