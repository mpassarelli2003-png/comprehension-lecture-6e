...
const STORAGE_KEY = "lecture6e_student_work_v6";
const OLD_KEYS = ["lecture6e_student_work_v5", "lecture6e_student_work_v4", "lecture6e_student_work_v3"];
...
function extractKeywords(prompt = "") {
  const stop = new Set(["quel", "quelle", "quels", "quelles", "dans", "avec", "pour", "pourquoi", "comment", "combien", "donne", "cite", "nomme", "explique", "texte", "selon", "peux", "vous", "nous", "sont", "etre", "est", "une", "des", "les", "aux", "sur", "qui", "quoi", "leur", "leurs", "cette", "cela", "avec", "deux", "trois", "quatre"]);
...
export default function Home() {
  ...
  function resetForExercise(nextExercise) {
    setExercise(nextExercise); setAdminJson(JSON.stringify(nextExercise, null, 2)); setStep(1); setQIndex(0); setNotes({}); setSelected(""); setProofs([]); setAnswers({}); setChecks({}); setReadingHighlights({}); setHighlighterActive(false); setView("student"); setSaveStatus("Nouveau texte commencé et sauvegardé.");
  }
  function importExercise() { try { resetForExercise(JSON.parse(adminJson)); } catch { alert("Le JSON contient une erreur."); } }
  function clearDraft() { if (typeof window !== "undefined" && window.confirm("Effacer les réponses sauvegardées sur cet appareil ?")) { [STORAGE_KEY, ...OLD_KEYS].forEach((key) => localStorage.removeItem(key)); setNotes({}); setProofs([]); setAnswers({}); setChecks({}); setReadingHighlights({}); setStep(1); setQIndex(0); setSaveStatus("Sauvegarde effacée."); } }
  function saveContextForVoiceHelp() { if (typeof window === "undefined") return; localStorage.setItem("lectureCurrentExerciseTitle", exercise.title); localStorage.setItem("lectureCurrentQuestionPrompt", question?.prompt || ""); window.location.href = "/aide-vocale"; }
  ...
  function useProofInAnswer(text) { const previous = answers[question.id] || ""; const prefix = previous.trim() ? "\nPreuve du texte : " : "Preuve du texte : "; setAnswers({ ...answers, [question.id]: previous + prefix + text }); }
  function clearQuestionProofs() { if (window.confirm("Veux-tu effacer tous les surlignages de cette question ?")) setProofs(proofs.filter((p) => p.questionId !== question.id)); }
  function toggleReadingHighlight(paragraphIndex, sentenceIndex) { if (step < 2 || !highlighterActive) return; const key = `${exercise.id}-${paragraphIndex}-${sentenceIndex}`; setReadingHighlights((old) => ({ ...old, [key]: !old[key] })); }
  function clearReadingHighlights() { if (window.confirm("Effacer tous les surlignages jaunes de lecture pour ce texte ?")) { const next = { ...readingHighlights }; Object.keys(next).forEach((key) => { if (key.startsWith(`${exercise.id}-`)) delete next[key]; }); setReadingHighlights(next); } }
  function renderParagraph(p, i) { const summary = String(notes[i] || "").trim(); const sentences = splitIntoSentences(p); return <div className="para" key={i}>{step > 2 && summary && <h3 className="paragraphSubtitle">{summary}</h3>}<p>{sentences.map((sentence, sIndex) => { const key = `${exercise.id}-${i}-${sIndex}`; const highlighted = !!readingHighlights[key]; return <span key={key} onClick={() => toggleReadingHighlight(i, sIndex)} className={`sentenceMarker ${highlighted ? "highlightedSentence" : ""} ${highlighterActive && step >= 2 ? "markerActive" : ""}`} title={highlighterActive && step >= 2 ? "Clique pour surligner ou retirer le surlignage" : "Active d’abord le surligneur jaune"}>{sentence}{" "}</span>; })}</p>{step === 2 && <input value={notes[i] || ""} onChange={(e) => setNotes({...notes, [i]: e.target.value})} placeholder="J’écris un sous-titre/résumé pour ce paragraphe..." />}</div>; }
