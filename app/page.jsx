...
const STORAGE_KEY = "lecture6e_student_work_v3";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const draft = { exerciseId: exercise.id, view, step, qIndex, notes, proofs, answers, checks, big, spaced, savedAt: new Date().toISOString() };
    localStorage.setItem(STORAGE_KEY, JSON.stringify(draft));
    setSaveStatus("Sauvegardé automatiquement sur cet appareil.");
  }, [readyToSave, exercise.id, view, step, qIndex, notes, proofs, answers, checks, big, spaced]);
  ...
  function clearDraft() {
    if (typeof window !== "undefined" && window.confirm("Effacer les réponses sauvegardées sur cet appareil ?")) {
      ...
      setNotes({}); setProofs([]); setAnswers({}); setChecks({}); setStep(1); setQIndex(0);
  ...
  function renderParagraph(p, i) {
    ...
    return <div className="para" key={i}>{step > 2 && summary && <h3 className="paragraphSubtitle">{summary}</h3>}<p>{p}</p>{step === 2 && <input value={notes[i] || ""} onChange={(e) => setNotes({...notes, [i]: e.target.value})} placeholder="J’écris un sous-titre/résumé pour ce paragraphe..." />}</div>;
  }

  const TopControls = () => <div className="card"><button onClick={() => setView("home")}>Accueil</button><button onClick={() => setView("choose")}>Changer de texte</button><button className="green" onClick={saveContextForVoiceHelp}>Aide vocale IA pour cette question</button><button onClick={() => setView("progress")}>Mes progrès</button><button onClick={clearDraft}>Effacer ma sauvegarde</button><p className="yellow"><b>{saveStatus}</b> Les réponses restent sur cet appareil même si tu utilises la flèche retour ou rafraîchis la page.</p></div>;
