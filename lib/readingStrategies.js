...
export function countEvidenceInStudentWork(work = {}, { exerciseId = "", modeId = "", questionIds = null } = {}) {
  ...
  const allowedIds = Array.isArray(questionIds) ? new Set(questionIds.filter(Boolean)) : null;
  const ids = new Set((Array.isArray(candidate.proofs) ? candidate.proofs : [])
    .filter((proof) => String(proof?.text || "").trim())
    .filter((proof) => !allowedIds || allowedIds.has(proof?.questionId))
