...
export function detectQuestionWord(prompt = "") {
  ...
  if (/\b(qu est ce|qu est ce que|qu est ce qui|quoi|que)\b/.test(value)) return "quoi";
...
export function countEvidenceInStudentWork(work = {}, { exerciseId = "", modeId = "" } = {}) {
  ...
  const ids = new Set((Array.isArray(candidate.proofs) ? candidate.proofs : []).map((proof) => proof?.questionId).filter(Boolean));
  return ids.size;
}
