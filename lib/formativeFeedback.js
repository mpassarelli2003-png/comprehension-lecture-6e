...
export const FORMATIVE_FEEDBACK_VERSION = "1.0";
...
export const FORMATIVE_MESSAGE_PREVIEW = {
  ...
  inferer: {
    ...
    partial: "Ajoute l’indice du texte qui t’a permis de faire cette déduction.",
...
export function estimateAnswerElements(answer = "") {
  ...
  if (strongSegments.length >= 2) {
    signals.push("liste ou séparateurs clairs");
    return { count: Math.min(strongSegments.length, 8), confidence: "high", signals };
  }
  ...
  if (connectorMatches.length) {
    signals.push("connecteurs d’énumération");
    return { count: Math.min(1 + unique(connectorMatches).length, 8), confidence: "medium", signals };
  }

  const andMatch = text.match(/^(.{4,})\bet\b(.{4,})$/);
  if (andMatch && words(andMatch[1]).length >= 2 && words(andMatch[2]).length >= 2) {
    signals.push("deux groupes reliés par et");
    return { count: 2, confidence: "low", signals };
  }
...
export function evaluateReadingAnswer(questionInput = {}, answer = "", evidence = {}, context = {}) {
  ...
  if (question.dimension === "inferer") {
    if (answerText && !evidenceState.hasEvidence) {
      strengths.push("Ton idée constitue une déduction possible.");
    } else if (evidenceState.hasEvidence && !hasExplainedLink) {
      strengths.push("Tu as trouvé un indice du texte.");
    } else if (answerText && evidenceState.hasEvidence && hasExplainedLink) {
      strengths.push("Ta déduction est reliée à un indice du texte.");
    }
  }
  ...
  const blockingMissing = normalizedMissing.filter((code) => [
  ...
  const publicMissing = blockingMissing.map((code) => ({
    code,
    message: modeId === "simulation" ? simulationMessage(code, question) : trainingNextStep(code, levelId, question)
  }));
...
export function evaluateExerciseSubmission(exerciseInput = {}, work = {}, context = {}) {
  ...
  const countMissing = (code) => results.filter((result) => result.missing.some((item) => item.code === code)).length;

  return {
    completeQuestions,
    partialQuestions,
    missingQuestions,
    missingEvidence: countMissing("evidence") + countMissing("evidencePrecision"),
    missingJustification: countMissing("justification") + countMissing("textLink"),
    missingElements: countMissing("elements"),
    missingPersonalPosition: countMissing("personal"),
    missingCriterion: countMissing("criterion"),
